(function() {
  ...
  module.exports = {
    ...
    'returns IPv6 zoneIndex': function(test) {
      ...
      test.equal(ipaddr.parse('2001:db8:f53a::1%2').toString(), '2001:db8:f53a::1%2');
    ...
    'returns IPv6 zoneIndex for IPv4-mapped IPv6 addresses': function(test) {
      ...
      test.equal(ipaddr.parse('::ffff:192.168.1.1%2').toString(), '::ffff:c0a8:101%2');
      test.equal(ipaddr.parse('::ffff:192.168.1.1%WAT').toString(), '::ffff:c0a8:101%WAT');
      test.equal(ipaddr.parse('::ffff:192.168.1.1%sUp').toString(), '::ffff:c0a8:101%sUp');
      return test.done();
    },
    ...
    'correctly converts IPv6 and IPv4 addresses to byte arrays': function(test) {
      ...
      test.deepEqual(ipaddr.parse('2a00:1450:8007::68%z').toByteArray(), [42, 0x00, 0x14, 0x50, 0x80, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x68]);
      return test.done();
    },
    ...
    'subnetMatch does not fail on IPv4 when looking for IPv6': function(test) {
      var rangelist;
      rangelist = {
        subnet6: ipaddr.parseCIDR('fe80::/64')
      };
    ...
    'subnetMatch does not fail on IPv6 when looking for IPv4': function(test) {
      var rangelist;
      rangelist = {
        subnet4: ipaddr.parseCIDR('1.2.3.0/24')
      };
    ...
    'subnetMatch can use a hybrid IPv4/IPv6 range list': function(test) {
      var rangelist;
      rangelist = {
        dual64: [ipaddr.parseCIDR('1.2.4.0/24'), ipaddr.parseCIDR('2001:1:2:3::/64')]
      };
